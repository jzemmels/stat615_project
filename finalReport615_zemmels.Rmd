...
```{r, echo = FALSE,message=FALSE,warning=FALSE,fig.cap='\\label{fig:cmcPlot} A visualization of the comparison results shown in the table above. CMCs are colored blue, non-CMCs are colored red, and NA cells are not shown.'}
load("~/cmas/crossValidation/topMatch_randomTrain/topMatch_largerTrainSet_train_CMCs/K015iZ1_vs_K017xZ3.RData")

load("matchScans.RData")
...
ggsave(filename = "kmOriginalMethod.pdf",plot = plt1,width = 5,height = 5)

invisible(knitr::plot_crop("kmOriginalMethod.pdf",quiet = TRUE))

knitr::include_graphics(path = "kmOriginalMethod.pdf")
```
...
```{r,echo=FALSE}
load("simulatedResults_modelz_entireFourBarrelsGFUR_initial.RData")
load("originalMethodCMCs_entireFourBarrels.RData")
```

```{r,echo=FALSE,fig.cap='\\label{fig:obsCMCCounts} The observed CMC count results for all 5151 comparisons between 102 scans. As we might expect, matching scans (scans fired from the same barrel) result in larger CMC scores on average.' ,fig.height=3,fig.width=5}
originalMethodCMCs_entireFourBarrels %>%
  ...
  theme_bw() +
  labs(x = "CMC Count",y = "Relative Frequency")
```
...
\begin{align*}
\text{logit}(p_{i,j,[k,l]}) = \underbrace{\alpha_0}_{\text{Intercept}} &+ \underbrace{\sum_{A \in \pmb{B}} \beta_A \cdot I\left(b(i) = A\text{ or }b(j) = A\right)}_{\text{Barrel-level Random Effect}} \\
...
The $\beta_A I(\hdots)$ terms behave like random intercepts if at least one of the two scans were fired from barrel $A$.
The $\gamma_{AC}I(\hdots)$ terms are the interaction between the different barrels.\footnote{Note that this model is slightly different from the one I provided in the original proposal. I ran into many issues with implementing the original model's random effect structure, so this was the closest that I could get to it.}
...
The model will be fit using the [`INLA`](https://www.r-inla.org/) package. The data and code used in this paper can be found at [a GitHub repo link here].

### Diagnostics

<!-- We will assess the convergence of the INLA procedure by considering diagnostics [XYZ]. -->

To determine quality of model fit, we will calculate a posterior predictive p-value for the CMC counts. Let $C_{i,j} = \sum_{k,l} y_{i,j,[k,l]}$ be the CMC count for the comparison between scan $i$ and scan $j$. As we are primarily interested in the barrel-level behavior of the CMC counts, we will consider the average of all CMC counts within each barrel pairing. More precisely, for two barrels $A,C \in \pmb{B}$, define the set of index pairs in which one scan was fired from barrel $A$ and the other scan from barrel $C$ as $\pmb{I}_{AC} = \{(i,j) : b(i) = A\text{ and }b(j) = C\text{ or }b(i) = C\text{ and }b(j) = A\}$. The average CMC count for the barrel pairing $AC$ is defined as:
$$
...
- For each comparison $(i,j) \in \pmb{I}_{AC}$:
...
- Compute the CMC count: $C_{i,j}^{(t)} = \sum_{k,l} y_{i,j,[k,l]}^{(t)}$

- Compute the average of the simulated CMC counts across $\pmb{I}_{AC}$: $\bar{C}_{AC}^{(t)} = \frac{1}{|\pmb{I}_{AC}|} \sum_{(i,j) \in \pmb{I}_{AC}} C_{i,j}^{(t)}$.

- Compute the posterior predictive p-value for this barrel pairing: $p_{AC} = \frac{1}{T} \sum_{t=1}^T I\left(\bar{C}_{AC}^{(t)} \leq \bar{C}_{AC}\right)$.
...
Due to memory constraints, a Gaussian approximation was used to estimate the conditional posterior distributions for the parameters given the hyperparameters (e.g., $p(\alpha_0 | \tau_0, \pmb{y}))$.\footnote{Using the more "expensive" Laplace approximation on a much smaller data set was maxing out my computer's 30 Gb of RAM and causing it to crash.}
...
```{r,cache = TRUE,echo=FALSE,eval=TRUE,fig.cap='\\label{fig:barrelEffects} Estimated posterior densities for the barrel and barrel interaction random effect terms. Higher-located densities correspond to comparisons that result in larger CMC counts.',fig.width=5,fig.height=3}
...
load("inlaFit_barrelRandomInteraction_modelz_entireFourBarrelsGFUR_initial_marginals.random.RData")
...
inlaFit_barrelRandomInteraction_modelz_entireFourBarrelsGFUR_initial_marginals.random$scanID[(329766 - 101):329766] %>%
  map2_dfr(.y = names(.),
           function(dat,name){
...
load("inlaFit_barrelRandomInteraction_modelz_entireFourBarrelsGFUR_initial_summary.random_scanID.RData")
...
load("simulatedResults_modelz_entireFourBarrelsGFUR_initial_marginals.random_cellID.RData")
...
```{r,echo = FALSE,fig.cap='\\label{fig:hyperparams} Estimated posterior marginal densities for the precision hyperparameters. Note the difference in the x-scale across each plot.',fig.width=5,fig.height=3}
...
load("inlaFit_barrelRandomInteraction_modelz_entireFourBarrelsGFUR_initial_marginals.hyperpar.RData")
...
As the problem mainly seems to lie in the estimation of the tail behavior, perhaps additional overdispersion corrections would help the model fit.
...
```{r,echo=FALSE,cache=FALSE,fig.cap='\\label{fig:obsSimComparison_facet} The simulated CMC counts faceted by the barrel pairing. Vertical lines are drawn at the observed average CMC count for each barrel pairing.'}
